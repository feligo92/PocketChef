...
let HomeComponent = class HomeComponent {
    ...
    ngOnInit() {
        ...
        this.subscription = this._data.allIngredientes.subscribe((newValue) => {
    ...
    addIngrediente() {
        let ingrediente = document.querySelector("#ingredientesInput").value;
        if (ingrediente !== '') {
            this.randomColor.push(this.colors[Math.floor(Math.random() * this.colors.length)]);
            //this.userIngredientes.push(ingrediente);
        }
...
let ListaComponent = class ListaComponent {
    constructor(_data) {
        ...
        this.subscriptionI = this._data.allIngredientes.subscribe((newValue) => {
            this.allIngredientes = newValue;
            for (let i = 0; i < this.userIngredientes.length; i++) {
        ...
        this.subscription = this._data.data.subscribe((newValue) => {
            ...
            this.arrRecetas.sort(function (a, b) { return b['puntuacion'] - a['puntuacion']; });
        });
        this._data.getIngredientes();
    }
    ngOnInit() {
...
let NuevaRecetaComponent = class NuevaRecetaComponent {
    constructor(_user, _data) {
        this._user = _user;
        this._data = _data;
        this.cantidades = [];
    ...
    addCantidad(event) {
        if (event['keyCode'] == 13) {
            let cantidad = document.querySelector("#cantidadesInput").value;
            let ingrediente = document.querySelector("#ingredientesInput").value;
            let flag = this.arrNombresIngredientes.includes(ingrediente);
            let ingredienteID;
...
NuevaRecetaComponent.ctorParameters = () => [
...
let RecetaComponent = class RecetaComponent {
...
RecetaComponent.ctorParameters = () => [
    { type: src_app_services_data_service__WEBPACK_IMPORTED_MODULE_2__["DataService"] },
